...
def home():
    ...
    st.info("""
            Source :
            [blog.barracuda](https://blog.barracuda.com/2020/03/26/threat-spotlight-coronavirus-related-phishing/)
...
def _paragraph():
    return bokeh.models.widgets.markups.Div(text=""" There are many reasons in today's environment to be wary of email that seems in any way suspicious.<br><br> 
                                            Some email messages might be phishing scams; some might contain viruses and other malicious software.<br><br>
                                            ...
                                            in order to steal money from him. In addition to phishing, spam emails can also take the form of advertising or a scam also known as "Nigerian Scam." <br><br>
...
"""
    ...
    email = ["Your email", "Median of regular spam*"]
    colors = ["#7ae0b3", "#b74355"]
    
    data = {'col' : col,
            'Your email'   : liste_param,
            'Median of regular spam*'   : liste1}
    ...
    p.vbar(x=dodge('col',  0.0,  range=p.x_range), top='Median of regular spam*', width=0.2, source=source,
           color="#5a6794", legend_label="Median of regular spam*")
...
"""
    ...
    p.vbar(x=dodge('col',  0.0,  range=p.x_range), top='Spam Median Value*', width=0.2, source=source,
           color="#7ae0b3", legend_label="Spam Median Value*")
